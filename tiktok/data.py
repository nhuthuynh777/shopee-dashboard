...
# Tất cả giá trị tiền trong TikTok (Cost, GMV, ATC, Checkout...) đều là THB → *830 → VNĐ
...
def clean_perf(raw):
    R = THB_TO_VND  # tất cả monetary field đều THB
    ...
    df['Currency_raw'] = pd.Series('THB', index=raw.index)

    df['Date'] = pd.to_datetime(raw['By Day'], errors='coerce') if 'By Day' in raw.columns else pd.NaT

    # Spend (THB → VNĐ)
    df['Cost_VND']         = _get(raw, ['Cost']) * R

    # Audience (count, không convert)
    ...
    # Shop metrics (THB → VNĐ)
    df['Purchases']        = _get(raw, ['Purchases (Shop)'])
    df['ATC']              = _get(raw, ['Adds to cart (Shop)'])
    df['ATC_Val_VND']      = _get(raw, ['Add to cart value (Shop)']) * R
    df['GMV_VND']          = _get(raw, ['Gross revenue (Shop)']) * R
    df['Checkouts']        = _get(raw, ['Checkouts initiated (Shop)'])
    df['Checkout_Val_VND'] = _get(raw, ['Checkout initiation value (Shop)']) * R
...
def clean_gmvmax(raw):
    R = THB_TO_VND  # GMV Max cũng là THB
    ...
    df['Purchases'] = _get(raw, ['Purchases (Shop)'])
    df['GMV_VND']   = _get(raw, ['Gross revenue (Shop)']) * R
    df['AOV_VND']   = _get(raw, ['Average order value (Shop)']) * R
